...
public class DungeonManager : MonoBehaviour {
    ...
    public class PathTile
    {
        ...
        public List<Vector2> GetAdjacentPath(int min, int max, Dictionary<Vector2, TileType> currentTiles)
        {
            ...
            if (min < position.x - 1 && !currentTiles.ContainsKey(new Vector2(position.x - 1, position.y)) 
                && type != TileType.essential)
                pathTiles.Add(new Vector2(position.x - 1, position.y));
            if (min < position.y - 1 && !currentTiles.ContainsKey(new Vector2(position.x, position.y - 1)))
                pathTiles.Add(new Vector2(position.x, position.y - 1));
    ...
    private void BuildEssentialPath()
    {
        ...
        while(boundTracker < maxBound)
        {
            ...
            if (ePath.position.x < nextEPath.position.x || ((nextEPath.position.x == (maxBound - 1)) && Random.Range(0, 2) == 1))
                ++boundTracker;
            ePath = nextEPath;
        }
        ...
        for(int x = (int)chamberOrigin.x; x < chamberOrigin.x + chamberSize; x++)
        {
            for(int y = (int)chamberOrigin.y; y < chamberOrigin.y + chamberSize; y++)
            {
                ...
                if(!gridPositions.ContainsKey(chamberTilePos) &&
                    0 < chamberTilePos.x && chamberTilePos.x < maxBound &&
                    0 < chamberTilePos.y && chamberTilePos.y < maxBound)
                {
